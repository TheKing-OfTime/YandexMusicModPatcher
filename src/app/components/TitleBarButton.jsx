...
function TitleBarButtonIcon({ variant, platform }) {
    if (platform === 'darwin') {
        switch (variant) {
            case 'quit':
                return (
                    <svg width="12" height="12" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" className="TitleBar_icon_darwin">
                        <circle cx="8" cy="8" r="8" fill="#FF605C" />
                        <g className="icon_darwin">
            ...
            case 'minimize':
                return (
                    <svg width="12" height="12" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" className="TitleBar_icon_darwin">
                        <circle cx="8" cy="8" r="8" fill="#FFBD44" />
                        <g className="icon_darwin">
            ...
            case 'maximize':
                return (
                    <svg width="12" height="12" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" className="TitleBar_icon_darwin">
                        <circle cx="8" cy="8" r="8" fill="#00CA4E" />
                        <g className="icon_darwin">
                            <rect x="4" y="4" width="8" height="8" />
                            <line x1="4" y1="4" x2="12" y2="12" stroke="#00CA4E" strokeWidth="3.5" />
                        </g>
                    </svg>
                );
            default:
...
function TitleBarButton({ variant, platform }) {
    const btnVariant = {};
    switch (variant) {
        case 'quit':
            btnVariant.id = 'quit';
            btnVariant.className = 'TitleBar_QuitButton' + (platform === 'darwin' ? '_darwin' : '');
        ...
        case 'minimize':
            ...
            btnVariant.className = 'TitleBar_Button' + (platform === 'darwin' ? '_darwin' : '');
            btnVariant.callback = React.useCallback(()=>{
    ...
    return (
        <button id={btnVariant.id} className={btnVariant.className} onClick={btnVariant.callback}>
            <TitleBarButtonIcon variant={variant} platform={platform}/>
        </button>
    )
}
